...
func TestLoadConfigOptional_CodexClampUnsupportedNone(t *testing.T) {
	dir := t.TempDir()
	configPath := filepath.Join(dir, "config.yaml")
	configYAML := []byte(`
codex:
  clamp-unsupported-none: true
`)
	if err := os.WriteFile(configPath, configYAML, 0o600); err != nil {
		t.Fatalf("failed to write config: %v", err)
	}
	...
	if !cfg.Codex.ClampUnsupportedNone {
		t.Fatalf("ClampUnsupportedNone = false, want true")
	}
}
