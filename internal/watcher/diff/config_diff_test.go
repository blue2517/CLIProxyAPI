...
func TestBuildConfigChangeDetails_FlagsAndKeys(t *testing.T) {
	oldCfg := &config.Config{
		...
		Codex:                         config.CodexConfig{ClampUnsupportedNone: false},
	...
	newCfg := &config.Config{
		...
		Codex:                         config.CodexConfig{ClampUnsupportedNone: true},
		ClaudeKey: []config.ClaudeKey{
	...
	expectContains(t, details, "codex.clamp-unsupported-none: false -> true")
